...
identification process.\note{Not in aid of--that form part of}  The pipeline can be used to compare new shark fin
images with existing images in a database, establishing whether the shark is
already on the database or not\note{whether the shark has been seen before}.  This information can be used to determine a more accurate population estimate in a specific region
...
valuable for biological and ecological research.\note{use attention line --- to
  connect to next sentence}Such information can be used to predict migration
patterns, possible habitats and also in warning people about \note{about the
...
this\note{this -> segmentatioon} a
non-trivial problem.  The approach\note{Waarna verwys jy met ``this approach''?} consists of
...
For this purpose, a database of shark fin images has\note{had already} been compiled.
To
draw reliable conclusions\note{reliable conclusions of what?}, images in the
database must be in a form such that it can be compared to the\note{scratch the} new images.
...
input data with existing images in the database.\note{tie sentences together
  with ``but''}  To perform such a
  process manually is laborious. 
Since the student's 
focus is on the distribution and movements of sharks and not on developing new
software, an algorithm that automates the task is a significant contribution. \\

Using such \note{such non-invasive methods?} technology, the Great White shark population and migration
patterns can be studied in a way that is not harmful to
the sharks, since the sharks need not to be tagged manually. \note{Next
  sentence is loose standing -- i.e., should not start with Thereby, perhaps
  tied to previous sentence.}Thereby
contributing in their conservation of sharks and the environment.  

\newpage
Below are a few examples of the data \footnote{I am indebted to
  Ms. S. Andreotti for providing us with \note{a sample from her carefully
...
\note{then this part can be trimmed}In other words each pixel is assigned a label and all the pixels with the same
...
example.
Furthermore, user interpretation is subjective since images can be
ambiguous\note{what does the statement ``images can be ambiguous'' mean?} and
the goal of the segmentation indicates the robustness of the algorithm\note{Not
  sure what you mean by the goal indicates the robustness?}. 
...
in medical imaging to locate tumours\note{locating tumours from medical images}, face and
fingerprint recognition and also in video surveillance.  See \ref{examples} for illustrations.
...
will rather be calculated.\note{Success rate of shark matches?  That does not
  tell us much about the segmentation.}

\subsection{Software -- Python}
\notes{Present tense, whenever possible...}
For the purpose of this research project, the Python programming language was
used\notes{because of its...}.  Reasons for this are because of its simplicity, the powerful set of tools
...
The procedure of identifying sharks can be
  ...
  a bigger classifier\note{each implemented as part of a bigger classifier?
    each pipeline? what does that mean?}.  The first algorithm is an orientation detection
...
\label{orient}
\note{Rephrase this.  To provide a consistent basis for further analysis, ...?
  Describe what this ``easing'' actually means.}To ease further analysis, the
...
the algorithm returns right and vice versa.\note{We're supposed to return the
  direction the shark is swimming.}  We then flip the images so that
all the shark fins have the same direction.  This will help when a few foreground
pixels and background pixels need to be specified when performing
segmentation.\note{not clear enough}
The method of Principal Components Analysis (PCA) is used here\note{to do what?
This happens a few times thoroughout the text--you mention a technique, but not
immediate how it is applicable.  That makes the reader scratch his/her head,
which leads to unhappiness.  Please your reader--give them the information they
need upfront.}.  When applying
PCA, we are interested in finding the direction of maximum variation in the
data\note{What data?  Data so far refers to images of sharks}.  Suppose we have $K$, $n$-dimensional, data points, $\mathbf{x}_k,$ for
...
In our case, the data refers to the pixel values in the shark fin
image.\note{See, here you tell me, but it is now too late!  I've already pulled
  out my hair!}\\
...
image, which is an RGB image, to a grayscale image.\note{Not necessary to state
  prev. sentence.}We then apply a Sobel filter
...
the image to determine the principal axes of the coordinates\note{of edge pixels?}.  If the gradient
...
initiated.\note{No description of growcut as an automaton?  Hoe kan ons die
  deel waar ek wel Growcut verduidelik hier in werk?? Sê ``The Growcut
  algorithm is discussed in detail in Section blabla''<-- en gebruik LaTeX se
  outomatiese verwysings sodat jy nie self nommers hoef in te tik nie}
Initially this was done by first determining the orientation of the fin (see
\ref{orient}) and then specifying a few pixels according to that\note{according
  to what?}, see 
\ref{segmentation}.  \\
...
of the image.  This then makes the algorithm more robust.\note{This does not
  make the algorithm more robust.  It just makes it work consistently across
  images of different sizes.  But tell the reader how you specify those coordinates.}  \\
...
how growcut works.  Hoe kan ons dit hier bywerk??? Als okay as jy vroeër 'n
verwysing ingesit het.}
\\
...
At the moment the segmentation algorithm is \note{taking a long time to execute.}running at a very low speed.
...
the results so that future requests for data can be served faster\note{This
  sounds like web architecture!  Just say that repeated calculations of the
...
finding the path defining the fin directly from the edge image.\note{Yes, but
  then also state that we'll have to find a different way of identifying the
...
to, do this, we need to extract this part of the fin, so that it is in a form to be 
compared with other fins easily.\note{Rephrase this sentence.}
We use the
Sobel operator,
well-known in edge detection, to identify edges in the image and \note{No
  ``easing'' without explaining how!}ease further analysis.
...
$x$-and\note{x-and?} $y$-direction respectively,
...
\ref{sobel}.  Note that this operator only works on a grayscale
image.\note{Probably no need to state this explicitly.}
We can see the edge of the fin in the $G$ image on the right clearly
identified.\note{There's a difference between identify and emphasize or highlight.}
...
them to increase their weight and thereby making them more prominent.\note{You
  need to explain why.  So, break up this sentence in two: explain why you
  invert, why you square separately.}  We make use of a function 
\texttt{route\_through\_array()}\cite{scikit} from the \texttt{graph} module in scikit-image.
 ...
 image.\note{What does ``lowest cost'' mean in this context?}
Since we are working on the segmented image, we allow any start and end point
for the path.\note{How do we do that? S: We set the first and last rows of the
  cost matrix = 0}  The path is computed by ??????.  All
pixel coordinates running on the edges of the image is then removed from the
path.  It is known that the lower you go down the fin, 
the less reliable\note{reliably} the the path is tracked.\note{Actually--it's
  just that the fin itself becomes less well defined, and then there is the
  bottom flappy part of the fin which can move between subsequent images.
  Ideally, we should make the bottom part count less than the top.}  This is, however, not a problem, since 
the fin will still be able to be identified, if we use only the top
$\frac{2}{3}$ of the fin for comparison.\note{Okay, here you explained it to
  some extent.} \\
...
Images such as the one below poses\note{pose} a great\note{scratch great} challenge when finding the path defining the dorsal fin.
Since the reflection of the fin on the water is so prominent, the path which is detected by this algorithm also
picks up the reflection.  We are therefore unsure \note{scratch as to}as to
where the path starts and ends.  This can be corrected \note{either by}by
either making sure the reflected part will be segmented out as background or by
shifting the path over the fin and detecting drastic changes in pixel
values.\note{This is way too dense to be understood--unpack.}
...
Dynamic time warping (DTW)\note{You can scratch the footnote now, since we are
  now using the implementation I wrote.}\footnote{I am indebted to Mr. M. de Klerk for providing us with a DTW implementation.} is an algorithm used for
measuring the similarity between two \note{similar} sequences which may vary in time or
speed.  It differs from normal comparison\note{what is a ``normal comparison''?} in the
...
between corresponding points.  The Euclidean metric can be used to calculate
this distance, but it assumes that the $i$-th point in the first sequence will
align with the $j$th point in the second sequence\note{This is wrong: Euclidean
  comparison assumes points have identical alignment--sorry, I might have
  misunderstood, so I think I changed this last time around.  Either way, you
  must explain that you have to do a point-wise comparison with Euclidean
  distance, whereas that won't work for varying length, time warped sequences}.  But since the sequences
...
each pair of elements in the array as entries.\note{You need to explain this at
  hand of $A_ij = dist(\bf{x}_i, \bf{y}_j)$} We now view this matrix as a
grid, where the
aim is to find a lowest cost path from the top left corner of the grid to the bottom right
corner of the grid.\note{Don't start sentences with thereby--rather join them together.}  Thereby making sure that each element in the array is compared to every other element in the array.
...
Here we show an example of calculating the distance matrix.
...
column.\note{Good--this should go in above where I mentioned that you need to
  describe how the cost matrix is formed.} \\
...
aquiring important data\note{ooh, important data :) what could that be?} about dolphins.  It was initially implemented
by \note{an}undergraduate student of
Eckerd College.
...
unreliable.\note{making the software unreliable for this specific purpose
  without further adjustment?  Pity we don't have a reference yet.}
...
required to encode that image, based on the given segmentation.
 \item \textbf{Histogram-based}\cite{threshold}  This is a very efficient method in the
sense that it only passes through all the pixels once\note{so does a simple
  threshold, of course.  In fact, I would just remove this, unless you
  want to explain all the intricacies}.  A histogram of 
...
sharing a certain property.\note{What property?}  The colour or intensity of the pixels can be used
to
locate clusters.\note{If this is what you're talking about, work it into the
  previous sentence.}
...
algorithms.
 \item \textbf{Region growing}\cite{threshold} This method takes as input not only
the image, but also a set of seeds, which marks the objects to be segmented. 
Thereafter neighbouring, unallocated\note{unallocated?} pixels are compared to the specific seed
point.  That is how the region grows iteratively.  The pixels are compared using
\note{measures such as} the difference between intensity value and the region's mean.  Pixels are then
allocated to a specific region \note{for which that...}if that difference is a minimum.  The process
terminates if all pixels \note{once all pixels are assigned to}belong to a a specific region.  \note{This is not
accurate, I
don't think.  Region merging is all about merging *regions*.  Don't
understand?? S: Region merging means taking an initial over-segmentation,
obtained using e.g. SLIC, and then merging those regions based on a set of
pre-determined properties, to form larger segments.}
...
graph is partitioned into clusters according to a specific\note{specified?} criterion.  Each
partition is then considered an object segment\note{object segment?} in the image.
...
two-dimensional \note{square grid of cells}grid of square cells.  Each cell can
be in \note{scratch either}either one of two states, dead  or alive.  In Conway's Game of Life, each
...
Below is an example of \note{scratch different}different structures created by
different rules, all following the procedure described above.\note{Different
  rules, or different initial conditions?}
...
the same and do not
change over time.\note{Some of this is now discussed above, so merge the common
  parts of these descriptions, or remove duplication here.}\\
...
$\theta_{p} \in [0,1]$. \note{This is all from the paper, but we do not need
  such a formal description.  Rather explain in your own words.}
...
\newpage
\noindent The pseudo code for the automata evolution rule is shown
below.\note{This is from the paper--cite it.} 
\begin{algorithm}[H]
\begin{algorithmic}[1]
 \State // For each cell...
 \For{$\forall p \in P$}
...
the edges.\note{The algorithm did not ``act more naturally''... the borders
  were just higher, so it was less likely to cross them--explain this intuition
  to the reader.}Next, the 'defend strength' of each pixel was modified also by
playing with exponents
of certain parts of the code.\note{super vague}  A
sobel filter was used in detecting the edges.\note{Detecting the edges? What
  edges? You don't mention that we call the algorithm on the edge image.}  A sobel filter calculates the
...
is that that part represents an edge.\note{Overlaps with earlier description of
  Sobel?}The figure below shows a sobel filter
...
edges.\note{What changes? How do you define accuracy?}  \\


\noindent Here is a comparison between the \note{result of rather than effect of?}effect of the original algorithm on
...
strategies have yet been implemented.\note{No idea what this paragraph means.} \\
...
and thus the rate of image labelling increases.\note{This belongs in the
  section that describes segmentation algorithms.}  \\
...
computed.\note{This sentence says nothing new?}  The latter is done by modelling the image as a weighted graph, where
...
pixel values are treated as a topography/landscape.\note{If the image is viewed
  as a landscape, the seed points are viewed as the locations of water sources.}The algorithm then floods
...
\note{Hoekom het jy hier 'n subsection nodig?}
...
Future work \note{can include -> entails}can include developing a user
interface, making the identification of shark fins more user friendly and
\note{scratch user}user interactive.
An user, who must be connected to some database of shark fins, will be able to
use this interface, containing the \note{acting as a frontend to the
  pipeline...}different parts of the pipeline, to identify shark fins.  Each
part will consist of an interactive "window".\note{Scratch prev. sentence}  The
different parts\note{results from the different sections of the pipeline can be
  inspected individually?}of the pipeline can be used separately, in
conjunction with each other or as a whole.  New shark fin images can then be
uploaded, from which various ecological and behavioural information can be
extracted.\note{Perhaps rather say that statistical queries and reports can be
...
running at optimum speed.  This then poses the challenge of speeding it
up\note{I think this sentence is obvious--scratch}.  Cython can be considered
as a possibility to rectify this\note{can be considered as a potential remedy}. \\
...
measured by the amount of correct matches it produces.\note{I still don't think
  we should tie the success of the classifier to the segmentation algorithm.
  This is currently only relevant for cutting out the path.}\\ 
...
One could then try to recognize various families of sharks.  The database could
then be updated to include various information about the sharks.\note{too vague}  \\
