...
\begin{abstract}
\note{This project investigates the use of various segmentation algorithms as
...
\note{Scratch this following sentence or integrate better.}This is accomplished
by analysing shark fin images using a computer.  The Growcut algorithm is used
as the main form of segmentation.\note{There are other forms of segmentation?
  Rather say something like the growcut algorithm is analysed in detail or so.}
...
already on the database or not.  From this, a more accurate population estimate
can be determined, thereby assisting in the conservation of Great White
sharks.\note{Too fluffy.  This is the most important paragraph of the
  report. Make it count!}
\end{abstract}
...
valuable for biological and ecological research.\note{Why?}  In order to do that and also to compare various shark sightings,
images of
shark fins\note{scratch the as will be shown later, replace will with ``are''}, as will be shown later, will be analysed by a computer. 
This analysis consists of identifying the edges in the image and segmenting the
foreground, the shark \note{scratch fin being most prominent}fin being most prominent,  from the background, the sea. 
...
non-trivial problem.  This project \note{scratch aims to}aims to investigate efficient segmentation
algorithms for shark fin identification.  \note{This sentence can be cut.}The approach consists of
...
The project \note{was conceived/started when}is motivated by a PhD
student in Marine Biology \note{who}that approached Dr. van der Walt for
...
To
\note{replace make with draw?}make \note{scratch any}any reliable conclusions, there has to be some form of identification in the
database.\note{This sentence can be rewritten.  There has to be some form of
  identification in the database?  Nothing happens ``in the database'' as
  such.  I know it's painful, but scientific writing requires precision!}
The goal is to group images of a certain individual together.  We
already know that it is possible to categorize each image, because \note{a
  human is able to group images based on the unique shape of} of the
uniqueness of the dorsal fin.  \note{To do so, one would have to...}One would just have to find a way to match new
input data with existing images in the database.  \note{To perform such a
  process by hand / manually is laborious.} One can only imagine the
difficulty in doing this manually. 
Since the student's 
focus is on the distribution and movements of sharks and not on developing new
software, \note{an algorith that automates the task is a significant contribution.}we can make a substantial contribution by researching this topic. \\

\note{Using such technology, }The Great White Shark population and migration
patterns can \note{scratch thus}thus be studied in a way that is not harmful to
the sharks, since the sharks need not to be tagged manually, thereby
contributing in \note{in their conservation}the conservation of sharks and the environment.  
...
\note{at instead of from?} from different locations.
Also note that not every image corresponds to a unique shark\note{every image
  *does* correspond with a unique individual--you want to say ``some sharks are
...
Image segmentation is commonly applied in image processing.  \note{join
  sentences together using a semi-colon}It
aims to partition a digital image into multiple segments \note{scratch the rest
  of this sentence}for some or other
reason.
The image can then instead be interpreted \note{as instead of using}using a small\note{smaller} number of
...
property.\note{join}  Thus making the image easier to analyse. Image segmentation is
...
an
image knowing only limited information about colour and space, for
example.\note{I know what you want to say in the previous sentence, but could
  be clarified.}
Furthemore\note{spelling}, user interpretation is subjective since images can be ambigious and the goal of the segmentation indicates the robustness of the algorithm. 
Supervised algorithms \note{remove therefore--this does not follow from the
  previous statement}therefore need user input to make decisions.  On the other hand,
...
do not perform as well as the supervised algorithms.\note{This does not
  necessarily follow.  You can say that supervised algorithms have a better
  chance of success.}  However, we strive to
develop more accurate, unsupervised algorithms.\note{Why do we strive for that?} \\

Well known examples where image processing\note{segmentation?} is used is
in medical imaging to locate tumours, for example, face and
fingerprint recognition and video surveillance.\note{Prev. sentence is broken}  See \ref{examples}.
...
Another challenge \note{scratch with image segmentation}with image segmentation is the evaluation of the resulting
segmentations.\note{I'd have rewritten the previous sentence, starting with
  Evaluating the segmentation outcome proves challenging.}  Often we don't have well defined criteria to evaluate the
segmentation. 
To make progress in this field \note{Scratch first part of this sentence.
  Start with ``It...''}it would be helpful to have a way for comparing
different
segmentation methods so that we know if progress is being made. \note{But it
  does not really add anything, so remove prev. sentence?}
The Berkeley Segmentation Dataset and Benchmark contains 12 000 hand-labelled
segmentations of 1 000 Corel dataset images from\note{from -> by?} 30 human subjects. 
...
that\note{Scratch the ``note that''} the image in the middle shows the machine segmentation and the
...
For the purpose of this research project, such dataset\note{datasets, or such a
  dataset}will not be used for
...
available \note{scratch in Python}in Python, \note{replace the fact by and}the
fact that it is easy to learn and \note{scratch rest, replace by understand}also easy for other 
people to understand.  Python is
also free to use and runs on multiple platforms.  For \note{scratch the}the numerical and
scientific computations, Numpy and SciPy was\note{were} used.
To improve the running time of \note{replace the with implemented}the algorithms, Cython was used.  It is a
...
variables and class attributes.\note{check last sentence}  This
results in \note{the generation of efficient}generating very efficient C-code from Cython code.
Scikit-image was used \note{to investigate various...}for the investigation of various image processing
techniques. \\

To ease collaboration\note{Ek maak met jou 'n deal--jy kan die woord ease 'n
  maksimum van 3
  keer in die hele verslag gebruik :)}, GitHub was used.  Github is a web-based hosting service
for open source software development projects\note{Nee, nie net vir Open Source
  projekte nie--ons s'n is dan nie open source nie!).  This allowed us to easily
share 
...
The aim is to put various algorithms together such that \note{Hoe kan ek
  hierdie mooi verwoord??}\note{The procedure of identifying sharks can be
  divided into several components, together forming an image processing pipeline.} each implemented as part of
  a bigger procesisng pipeline.}  The first algorithm is an orientation detection
algorithm \note{that wil->used to}that wil determine whether the shark fin is orientated to the left or the right in the image.
Then a segmentation algorithm will segment the shark fin from the image
such\note{such -> so} that the path that defines the shark fin 
can easily be detected by the fin detection algorithm.\note{Skaaf bietjie aan
  hierdie sin. ... so that the fin detection algorithm can focus on the fin
  itself without having to be concerned with background noise due to the ocean?}}  Thereafter a dynamic time warping algorithm is used
to compare the the paths that describe the \note{coordinates of the
  different}fins and to \note{compare it to other fins in the database}see how shark fins in the databasis compare.
The various algorithms will now be discussed in detail.  Below is a diagram showing the structure of the pipeline.

\begin{figure}[H]
 \centering
 \includegraphics[width=5in]{pipeline.jpg}
\end{figure}
