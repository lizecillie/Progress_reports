...
\subfigure[]{\includegraphics[width=1.7in]{face.jpg}}}
...
\texttt{route\_through\_array()}\cite{scikit} from the \texttt{graph} module in scikit-image.
 ...
 image.  The cost of the path is defined as the sum of the values of the array along the path.
...
\begin{figure}[H]
\centering
\mbox{\subfigure[]{\includegraphics[width=2in]{haai2.jpg}} \quad
\subfigure[]{\includegraphics[width=2in]{haai3.jpg}}}
...
each pair of elements,  $(\bf{x}_i, \bf{y}_j)$, in the array as entries.
...
algorithms.
 \item \textbf{Region merging}\cite{threshold} Region merging means taking an initial 
 over-segmen-tation,
...
\item \textbf{Region growing} A semantic segmentation
...
\label{ca}
The idea of a cellular automaton is explained with the use of an example.
...
\noindent The pseudo code for the automata evolution rule as described in\cite{alg}.
\begin{algorithm}[H]
\begin{algorithmic}[1]
 \State // For each cell...
 \For{$\forall p \in P$}
...
the segmentation improves.  Since the edges of the shark fin now 
  have a higher value, the algorithm is less likely to cross them. \\  
...
\begin{figure}[H]
\centering
\mbox{\subfigure[]{\includegraphics[width=1.6in]{findetect1.jpg}} \quad
\subfigure[]{\includegraphics[width=1.6in]{findetect2.jpg}} \quad
\subfigure[]{\includegraphics[width=1.6in]{findetect4.jpg}}}
\end{figure}

\begin{figure}[H]
\centering
\mbox{\subfigure[]{\includegraphics[width=1.6in]{findetect3.jpg}} \quad
\subfigure[]{\includegraphics[width=1.6in]{findetect5.jpg}} \quad
\subfigure[]{\includegraphics[width=1.6in]{findetect6.jpg}}}
\end{figure}

There are however fins, that are similar to the ones below, which contain a lot of noise and thereby influencing the detection of the fin.
This could result in the wrong side of the fin being detected.  To correct this, there is the option of detecting the paths that describe each side of the fin
and then determining which path has the greatest variance.  Since we expect the serrated part of the fin to have greater variance, this method will hopefully indicate which path to use for fin matching. \\

\begin{figure}[H]
\centering
\mbox{\subfigure[]{\includegraphics[width=1.6in]{findetect7.jpg}} \quad
\subfigure[]{\includegraphics[width=1.6in]{findetect8.jpg}} \quad
\subfigure[]{\includegraphics[width=1.6in]{findetect9.jpg}}}
\end{figure}
...
The Growcut algorithm is discussed as the main segmentation algorithm.  Orientation detection, segmentation, fin detection and fin matching form the building blocks of an image processing pipeline.   The pipeline is unable to accurately match all shark fins in the database. \\
...
A user, who must be connected to some database of shark fins, will be able to
...
Some pre-processing of the shark fin images might be useful in improving the accuracy of the pipeline.  Since the images are very noisy, 
...
of the fin, colour spaces other than RGB might prove to be valuable when detecting edges. 
Investigating the various parameters in the Growcut algorithm, like the window size, the damping function and attacking strategies, could also lead to the improvement of the pipeline.
 \\
...
Recognizing various families of sharks can then be attempted.  
