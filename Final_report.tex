...
already on the database or not.  This information can be used to determine a more accurate population estimate in a 
specific region, thereby assisting in the conservation of the Great White shark.
\end{abstract}
...
valuable for biological and ecological research.  Such information can be used to predict migration patterns, possible habitats and also warn people about sharks in a certain region.  In order to identify the sharks and also to compare various shark sightings,
images of
shark fins are shown later, will be analysed by a computer. 
...
student in Marine Biology, Ms. S. Andreotti, who approached Dr. van der Walt for
help regarding a problem she encountered in her research, namely how can images of shark fins be used in identifying sharks so that their
...
To
draw reliable conclusions, images in the database must be in a form such that it can be compared to new images.
...
human is able to group images based on the unique shape of the the dorsal fin. 
To do so, one would just have to find a way to match new
input data with existing images in the database.  To perform such a
process manually is laborious.
Since the student's focus is on the distribution and movements of sharks and not on developing new
...
automatically.
However, we still strive to
develop more accurate, unsupervised algorithms, in order to develop faster methods that can interpret large sets of data easily. \\
...
Evaluating the segmentation outcome proves challenging.  
It would be helpful to have a way for comparing
different
...
available and it is easy to learn and understand.  Python is
...
results in the generation of very efficient C-code from Cython code.
