...
It differs from normal comparison in the sense that it accommodate sequences of different lengths, warping the sequences for the best fit.
Any data that can be represented linearly can be analysed with DTW.  \\

For the comparison, the Euclidean metric can be used, but this metric assumes that the $j$th point in the first sequence will allign with 
the $j$th point in the second sequence.  But although the . \\

For this part of the pipeline, the output from fin detection is used as input.
...
The lower the cost, the better the fit. For every fin compared to every other fin in the database, a cost is calculated. The fin corresponding to the 
lowest cost is then considered a match. \\
