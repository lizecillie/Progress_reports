...
dorsal fin structure.  Identifying shark sightings \note{Why these days?}these days can be very
valuable.  In order to do that and also to \note{What does it mean to "match
shark sightings?"}match shark sightings, photos of
shark fins, as will be shown in the proposal, will be analysed by a computer. 
\note{We prefer to write in present tense.}This will consist of comparing the edges in the image and segmenting the
foreground\note{Comparing the edges to what?}, the shark fin being most prominent,  from the background, the sea. 
Although the photos provided are of a good quality and \note{have been cropped
to only include the fin...}only include the fin, the
...
algorithms for shark fin identification.  The approach \note{present tense}will consist of
...
\note{Write a bit more formally: the project is motivated by...}The idea behind the project originated when PhD
...
Wil ook meer se oor die impak van so projek en ook oor die bewaring van die haaie.

\newpage
Below are a few examples of the data used in this research project.  Wil meer
se oor die databasis???\note{Verduidelik hoe die databasis opgestel word,
dalk?}
...
vary significantly.\note{Already stated earlier.}


\subsection{Background}
Image segmentation is well known in the field of image processing.\note{This
    makes it sound like you're talking about a person.  Rather: image
segmentation is commonly applied in image processing.}  It consists\note{It
aims to...}
...
automatic.\note{supervised or supervised?}  Segmentation is a hard
problem\note{Citation or motivation required.}.  Computers must segment an
imgae\note{spelling--use aspell or ispell}
knowing only limited information about colour and space, for example.
Many images can be very ambigious\note{in what sense?} and supervised algorithms need user input to make decisions.  On the other hand, unsupervised algorithms need predetermined information and assumptions to work automatically.
It then follows natuarally\note{few things ever follow naturally...is this
statement even true?  If so, why?  Motivate.} that the unsupervised algorithms do not perform as well as the supervised algorithms.  However, we strive to develope accurate,
unsupervised algorithms. \\

Well known examples where image processing are\note{are/is} used is
...
segmentations. Segmentation is subjective -it's based on people's
opinions\note{remove the last part of this sentence; not sure I
    agree--segmentation based on a well-defined criterium is not subjective,
    it's just that often we don't have well-defined criteria.  So think
carefully about what you want to say here.}.
To make progress in this field there has to be\note{Remove "has to
be"--perhaps would be "helpful to have"?} a way for comparing different
segmentation methods so that we know if progress is being made. 
The Berkeley Segmentation Dataset and Benchmark contains 12 000 hand-labeled segmentations of 1 000 Corel dataset images from 30 human subjects. 
This can then successfully \note{remove successfully--the success depends a
lot on who does the job}be used to compare what humans think a good segmentation should look like. \\

Here is an example of a colour image in the Berkeley Segmentation Dataset.  Note that the image in the middle shows the machine segmentation and the
...
comparison of results.  How???\note{What are you asking/saying with this
paragraph?}
...
used.  The reason for this is mainly because of its simplicity\note{it's not
    onyl for the simplicity--it is because of the powerful set of tools
    available, because it is easy to learn, and because the resultant code is
easy to understand also to other students}.  Python is
also free to use and runs on multiple platforms.  For the numerical and
scientific computations, Numpy and SciPy was used respectively.\note{scratch
"respectively"}
To improve the running time of the algorithms, Cython was used.  It is a
language that compiles to a Python extension module.\note{join sentences ->
and a}  It is a super set of the
...
sentence}  For the investigation of various image processing algorithms, scikit-image was used. \\
Nog iets????\note{dink dit is dit, maar noem ook iets van die
ontwikkelingsproses op GitHub}
...
\label{orient}
To ease further processes\note{processes?} and analysis, we determine the
orientation of the shark fin in the image in terms of\note{not in terms of--we
determine the absolute direction; otherwise the answer would always be--the
orientation of the shark is in the direction it is swimming!}the direction in
which the shark is
...
identified as
"right", otherwise \note{scratch up to end of "as"}the orientation is identified as "left". 
...
Initially this was done by first determining the orientation of the fin (see \ref{orient}) and then specifying a few pixels according to that, see 
\ref{segmentation}.  \\

\begin{figure}[H]
 \centering
 \includegraphics[width=5in]{segmentation.jpg}
 \caption{Growcut segmentation on a shark fin image by selecting a few foreground and background pixels, shown in blue and red respectively.}
 \label{segmentation}
\end{figure}

However, more success was achieved by rather reflecting the fin in the
horisontal direction to ensure that all the shark fins have the same
...
\ref{segmentation1}.
Since the images in the databases, or in general,\note{or in general?} are not
all the same size, we specify the corners of the polygon according to the size
of the image.\note{Show the triangles you define}
Also making this algorithm more robust.\note{not a full sentence}  \\

\begin{figure}[H]
 \centering
 \includegraphics[width=5in]{segmentation.jpg}
 \caption{Growcut segmentation on a shark fin image by selecting all points in a given polygon as foreground and background pixels, shown as blue and red
...
These parameters inevatably\note{spelling} influence the success of the
...
\note{show at different stages, perhaps?}
...
compared with other fins\note{broken sentence}.  We use the Sobel operator,
...
masks, $M$, which are convolved with the original image to approximate the
...
We have now managed to successfully identify the edges in the
image.\note{Scratch sentence} \\
...
\texttt{route\_through\_array()} from the module \texttt{graph} in scikit-image.  This function enables us to find a lowest cost path through the image.
...
Here we show an example of the path, in blue, that was detected on the shark fin.
\begin{figure}[H]
 \centering
...
When comparing two sequences, \note{a typical approach is to...} the distance
between corresponding points is calculated.  The Euclidean metric can be used
to calculate this distance, but
it assumes that the $j$th\note{dalk soek j-th 'n koppelteken? -- dink ook jy
moet i-th hier gebruik, en dan wys dat dit 'n inskrywing $A_ij$ vorm} point in
the first sequence will allign with the $j$th point in the second sequence.
But
since the sequences need not be
the same length, we will make use of a non-linear alignment\note{explain}.

A distance matrix is calculated, containg\note{containg!} the distance between
...
corner.\note{why?}
The minimum cost is then a meassure of the similarity between the
...
other shark fin in the databasis, each time returning a minimum cost.  
Thus for a databasis containing $N$ shark fin images, a $N \times N$ cost
...
aquiring important data about Great White dolphins\note{must be an interesting
species}.  It was initially implemented by undergraduate student\note{we live
    in a meritocracy now--no need to mention his position way down the food
chain!}Mark Allen of
Eckerd College under the
supervision of John Stewman.
Lately it was used to estimate the Great White shark population in the
Gansbaai area by means of shark fin identification and matching.  This
claim\note{what claim?}
was tested by Ms. Andreotti and she found that in only
54\% of the cases the correct matching took place, making this software highly
unreliable\note{highly unreliable is a judgement, unreliable is a statement}.
Below is an image showing the DARWIN user interface.\note{no need} 
...
\begin{itemize}
 \item \textbf{Thresholding}  Possibly the most basic method in the
     field\note{Don't be so harsh on thresholding!  This is a judgement call.
     Just state the facts: it is an unsophisticated method}.  It relies on
     selecting the best\note{again "best"--what does best mean?} threshold
     value to convert a gray-scale image into a binary image.  Segmentation is
...
segmentation will follow.\note{This is one approach--don't present it as the
default.}
...
can be used as a measure, are then used to locate clusters in the
image.\note{explain--I don't understand the idea}
 \item \textbf{Edge detection} Edge detection is a well-developed\note{Edge
     detection is well researched in image processing.}
...
\note{References to all of these--above and below!}
...
can be in a finite number of states,\note{In the case of segmenting an image,
    we typically use a two-state automaton, where the states indicate
foreground and background.}say on and off.  This must be specified
beforehand.  Around each cell, a set of cells, called the cell's neighbourhood,
is defined\note{exists?}.  An initial state, at time $t = 0$, is also assigned
to each cell.  A
...
details on how these rules are formed for segmentation}  It is known that the same rules apply to each cell and do not change over time.
...
\noindent It can clearly be seen\note{nothing in science can ever be clearly
seen} that the modified version gives better results around the edges, which is essential in the case of the shark fin images. \\ 
...
random walker to see how they explained it.}  In other words, if the user
