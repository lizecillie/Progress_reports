...
\newpage
Below are a few examples of the data \footnote{I am indebted to Ms. S. Andreotti for providing us with the necessary data.} used in this research project.  
...
\note{Kan jy eerder 'n subfigure gebruik hier?  Dan hoef jy ook nie bo 'n
  manual new page in te sit nie.  Ek gebruik dan subfigures???}
\begin{figure}[H]
\centering
...
example.
Furthermore, user interpretation is subjective since images can be ambiguous and the goal of the segmentation indicates the robustness of the algorithm. 
...
the less reliable the the path is tracked.  This is, however, not a problem, since 
the fin will still be able to be identified, if we use only the top $\frac{2}{3}$ of the fin for comparison. \\
...
Dynamic time warping (DTW)\footnote{I am indebted to Mr. M. de Klerk for providing us with a DTW implementation.} is an algorithm used for
