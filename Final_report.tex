...
Joblib is a set of tools providing lightweight pipe lining in Python. 
...
The above method proved unsuccessful in many cases.  The reason is because the Sobel edge detector captures 
...
in mind that we will then have to find a different way of identifying the
  actual segment that belongs to the fin itself.  For the purpose of this research project,
  no segmentation will be performed. 
...
and then there is the
  ...
  We therefore make the bottom part count less than the top by only using the top
...
Detecting the fin as described above is a time consuming process.  For this reason, the results of this part of the
pipeline are cached using the Joblib software package.
...
The DTW implementation\footnote{I am indebted to
  Dr. S.J. van der Walt for providing us with a DTW implementation.} we use, only accommodates
...
sequences.  Note that the minimum costs are normalized. \\
...
since the fins become more unreliable as we move closer to the body of the shark.  The sharks shown below are a few of which the fin were detected very successfully.  \\
...
There are however fins like the ones below which contain a lot of noise and thereby influencing the detection of the fin. 
...
Below is the first five rows and five columns of the lower triangular cost matrix
\[
 C = \begin{bmatrix*}
 & 0 & 0 & 0 & 0 \\
 & & & & \\
...
Below is the image of the complete $50 \times 50$ cost matrix.  The lower the entry in the cost matrix, the darker the pixel and the closer the match.  \\
...
Here are a few examples of sharks from the database together with their closest match. \\
...
noise removal is an option.  Also, because we are only interested in the pattern describing the serrated part
