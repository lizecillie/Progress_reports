...
This week the main focus was on an article \cite{growcut} about the Grow Cut segmentation algorithm.  A Python implementation of this
algorithm \cite{alg} was received by St\'{e}fan, with the purpose of including it in the Scikit-Image package mentioned in Progress report 1.  The accompanying article was 
studied in great detail to determine how the algorithm works and how it can be optimized in such a way as to improve the segmentation of our shark fin images.  Also note that some
of the future work mentioned in Progress report 1, has not been done yet.
The working of the algorithm as well as possible optimization will now be discussed.

\section{Discussion}
The Grow Cut algorithm is an interactive, multi-label segmentation algorithm for N-dimensional images.  The algorithm is based on cellular automata, ie.,  the user labels a few pixels
